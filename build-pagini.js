...
const { pagina, grilaHTML, FILTRE, FIRMA, esc, scrie, UG } = B;
...
const INTREBARI = [
  ...
  ['Cât durează deschiderea ușii?',
   'Timpul de deschidere sau de închidere este de aproximativ 10 secunde.'],
...
function cardIdentitate(base) {
  ...
  <dl class="idcard__body">
    ...
    <div class="idrow"><dt>Magazin</dt><dd><a href="${ORIGINE}/" rel="noopener">usa-garaj.ro</a></dd></div>
  </dl>
</div>`;
}
...
const contactHTML = (base) => `<div class="contact-grid">
  <div class="contact-cta reveal">
    <div class="contact-lines">
      ...
      <a class="contact-line" href="${ORIGINE}/magazin/" rel="noopener">
        ...
        <div><small>Comandă online</small><b>Magazinul Usa-garaj.ro</b></div>
      </a>
    </div>
    ...
    <div class="produs">
      <div class="produs__media">
        ...
        <figure class="produs__foto">
          <img src="../${UG.fotoProdus(p)}" alt="Fotografie ${p.familie === 'antracit' ? 'ușă de garaj gri antracit' : 'ușă de garaj maro'}" loading="lazy" decoding="async">
          <figcaption>Fotografie din magazin · ${raluri}</figcaption>
        </figure>
      </div>
      ...
      <div class="produs__info">
        ...
        <div class="table-scroll produs__spec">
          <table class="spec">
            ...
            <tbody>
              <tr><th scope="row">Dimensiune totală (L × H)</th><td>${p.l} × ${p.h} mm</td></tr>
              <tr><th scope="row">Pasul lamelei</th><td>${geo.pas} mm</td></tr>
              <tr><th scope="row">Grosimea lamelei</th><td>${String(geo.grosime).replace('.', ',')} mm</td></tr>
              <tr><th scope="row">Culoare</th><td>${raluri}</td></tr>
              <tr><th scope="row">Casetă din aluminiu</th><td>${geo.caseta} mm</td></tr>
              <tr><th scope="row">Ghidaje din aluminiu</th><td>${geo.pas === 55 ? '75 × 30 mm' : '90 × 35 mm'}</td></tr>
              <tr><th scope="row">Ax din oțel zincat</th><td>Ø ${geo.ax} mm</td></tr>
              <tr><th scope="row">Masa tabliei</th><td>${geo.masa} kg/m²</td></tr>
              <tr><th scope="row">Acționare</th><td>motor tubular, centrală de comandă, 2 telecomenzi</td></tr>
              <tr><th scope="row">Acționare de rezervă</th><td>manivelă și legătură cardanică</td></tr>
              <tr><th scope="row">Timp deschidere / închidere</th><td>circa 10 secunde</td></tr>
              <tr><th scope="row">Cod produs</th><td>${p.id}</td></tr>
            </tbody>
          </table>
        </div>

        <div class="produs__cta">
          <a class="btn btn--primary btn--lg" href="${UG.urlProdus(p)}" rel="noopener">Comandă în magazin</a>
          <a class="btn btn--ghost btn--lg" href="tel:${FIRMA.telHref}">Sună la ${FIRMA.tel}</a>
        </div>
        <p class="produs__nota">Denumirea, prețul și disponibilitatea sunt preluate întocmai din magazinul usa-garaj.ro.</p>
      </div>
    </div>
  </div>
</section>
...
async function paginiContinut() {
  for (const [slug, titlu] of PAGINI_CONTINUT) {
    ...
    const continut = areText ? corpText : `
      ...
      <ul>
        ...
        <li><a href="${ORIGINE}/${slug}/" rel="noopener">Versiunea de pe usa-garaj.ro</a></li>
      </ul>
  ...
  const urls = scrise.filter((f) => f.endsWith('.html'))
    .map((f) => `  <url><loc>${ORIGINE}/${f.replace(/\\/g, '/')}</loc><lastmod>${azi}</lastmod></url>`)
    .join('\n');
  scrie('sitemap.xml',
    `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls}\n</urlset>\n`);
  scrie('robots.txt', `User-agent: *\nAllow: /\nSitemap: ${ORIGINE}/sitemap.xml\n`);
